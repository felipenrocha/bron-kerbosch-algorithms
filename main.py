...
def main():
    ...
    P_incial = get_all_vertex(lista_adjacencia_golfinhos)
...
#  R: vértices que seriam parte do clique
#  P: vértices que têm ligação com todos os vértices de R (candidatos).
#  Conjunto X: vértices já analisados e que não  levam a uma extensão do conjunto R.
    ...
    for v in P[:]:
        ...
        R_next = R.append(v)
        # R_next.append(v)
        P_next=intersection(P, neighbor(v, lista))
        X_next=intersection(X, neighbor(v, lista))

        bron_kerb_algorithm_no_pivot(R = R_next, P = P_next, X = X_next)
        P.remove(v)
        X.append(v)
